...
public partial class HowToCecilTests(ITestOutputHelper output)
{
    ...
    public void WhenOpeningModuleAndReferencingItsTopLevelType()
    {
        ...
        // Assert.
        Assert.Contains("System.Threading.ReaderWriterLock", console.GetOutput());
    ...
    public void WheCheckIfATypeHasACertainCustomAttribute()
    {
        ...
        Assert.Equal(typeof(Foo.IgnoreAttribute).FullName, ignoreAttribute!.AttributeType.FullName);
        Assert.Single(ignoreAttribute.ConstructorArguments);
        Assert.Equal("Not working yet", ignoreAttribute.ConstructorArguments[0].Value);
...
#if DEBUG
        Assert.Equal(9, method.Body.Instructions.Count);
        Assert.Equal(OpCodes.Add, method.Body.Instructions.FirstOrDefault(x => x.OpCode == OpCodes.Add)?.OpCode);
#else
        Assert.Equal(5, method.Body.Instructions.Count);
        Assert.Equal(OpCodes.Add, method.Body.Instructions.FirstOrDefault(x => x.OpCode == OpCodes.Add)?.OpCode);
#endif
...
#if DEBUG
        Assert.Equal(11, method.Body.Instructions.Count);
#else
